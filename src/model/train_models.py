...
try:
    ...
    print("Warning: CatBoost not available. Skipping CatBoost model training.")
...
class CropYieldModelPipeline:
    ...
    def train_ensemble_models(self, data):
        ...
        if CATBOOST_AVAILABLE:
            ...
            print("\n5. CatBoost - SKIPPED (not available)")
    ...
    def error_analysis(self, model_name, y_true, y_pred):
        """Print top 10 largest errors and percent errors, and save to file"""
        y_true = np.array(y_true)
        y_pred = np.array(y_pred)
        ...
        print(f"  Mean Absolute Error: {overall_stats['mean_absolute_error']:.2f}")
        print(f"  Mean Absolute % Error: {overall_stats['mean_absolute_percent_error']:.2f}%")
        print(f"  Max Absolute % Error: {overall_stats['max_absolute_percent_error']:.2f}%")
        print(f"  Std of % Errors: {overall_stats['std_percent_errors']:.2f}%")
        ...
        for low, high in ranges:
            if high == np.inf:
                ...
                print(f"  {low}+%: {count} samples ({percentage:.1f}%)")
            else:
                ...
                error_distribution.append(dist_data)
                print(f"  {low}-{high}%: {count} samples ({percentage:.1f}%)")
        ...
        error_analysis_dir = self.models_dir / "error_analysis"
        error_analysis_dir.mkdir(exist_ok=True, parents=True)
        
        model_file_name = model_name.replace(' ', '_').replace('-', '_').lower()
        json_filepath = error_analysis_dir / f'{model_file_name}_error_analysis.json'
        
        with open(json_filepath, 'w') as f:
            json.dump(error_analysis_data, f, indent=2)
        
        print(f"  ✓ Saved error analysis to {json_filepath}")
